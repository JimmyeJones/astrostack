...
- **LEAD (Builder 2026-09-18, filed with v0.456.0/v0.456.1 — the third surface of the same shortcut, and the
  only one that changes pixels rather than sentences) — `_auto_bind_for_target` picks the unattended stack's
  master dark from the target's *median* sub exposure.** *(Pillar: autonomy + image quality — PRIORITY 2/4;
  size S to write, **M to be sure of** — it is the walk-away hot path. Confidence: the median is traced
  (`webapp/pipeline.py`, `exposure_s=_med([f.exposure_s for f in frames])`); how often the owner's targets
  are actually mixed is **not measured**.)*
  v0.456.0 and v0.456.1 fixed the two surfaces that *talk* — the run's advisory and the Stack form's
  pick-time caution — so a mixed-exposure target is now named honestly in both. This one *acts*: on a target
  shot at 10 s on one night and 30 s on the next it binds a dark matched to the median and the minority subs
  get it subtracted unscaled, which is exactly what the advisory now reports after the fact.
  **Why it is filed rather than built, and the shape that is probably right.** The obvious move — decline to
  bind when no single dark fits the spread — is the rule the function already applies to a mismatched
  median ("leaves the dark off rather than risk an over/under-subtraction"), but applied here it would strip
  calibration from a target that is 95 % one exposure because 5 % of it is another, which is worse overall.
  The better shape is the one the function *already* reaches for on a mismatched median: when the subs are
  mixed and a confident master bias exists, bind `scale_dark_to_light=True`, because `apply_raw` is handed
  each frame's own exposure and the scaling path is therefore per-frame correct on a mixed target by
  construction. **That changes pixels on the on-by-default walk-away path**, so it wants: a measurement of
  how many of the owner's targets are genuinely mixed (the observer can answer this from the frame rows —
  `distinct_exposures` over each project's accepted subs is the whole query), and the fallback left exactly
  as today whenever no confident bias exists. Do not flip it blind.
