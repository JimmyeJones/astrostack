...
- **NEW IDEA (Builder 2026-08-18, the pattern behind both v0.266.1 and the hints added in v0.267.0) — a Tooltip is
  ...
  *invitation* and a hover-only reply to them is a dead link. Hover is byte-for-byte what it was; keyboard users
  ...
  `History.test.tsx`.
  **Still open after it:** the ~40 remaining non-control anchors, of which the **Tonight page's five planning
  badges** are the most phone-critical in the app (you read that page standing next to the scope) — plus
  `SuggestTargetsCard`, `ContinueTonightCard`, `NextSessionCard`, `NightsCard`, `BestMonthsStrip`,
  `MosaicMapCard`, the editor's `OpList` chips, and the `Tonight`/`Target`/`Gallery` badges. `HintAnchor` makes
  each of those a one-line swap; take them a coherent surface at a time, not as a sweep.
