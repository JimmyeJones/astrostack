...
/** One target whose newest picture was a *degraded* one — a storage hiccup let a
 * thin stack get published before the readability guard existed — and which this
 * scan re-stacked back to the full set now that every sub is readable again
 * (`auto_stack_healed`). Worth saying out loud: a picture silently getting
 * *better* is as confusing as one silently getting worse. */
export interface HealedTarget {
  target: string; frames: number; previous: number; best: number;
}
...
export function pipelineSummary(r: Record<string, unknown>): {
  ...
  const healed: HealedTarget[] = Array.isArray(r.auto_stack_healed)
    ? (r.auto_stack_healed as unknown[])
        ...
        .map((o) => ({
          target: typeof o.target === "string" ? o.target : "",
          frames: Number(o.frames ?? 0) || 0,
          previous: Number(o.previous ?? 0) || 0,
  ...
  if (healed.length > 0) {
    clauses.push(`re-made ${healed.length} thin picture${healed.length === 1 ? "" : "s"}`);
  }
      ...
      <Stack gap={4} mt="xs">
        ...
        {healed.length ? (
          <Alert color="teal" variant="light" p="xs"
            ...
            <Text size="xs">
              {healed.length === 1 ? "One of your targets" : `${healed.length} of your targets`}
              {" had a newest picture made from far fewer subs than an earlier "}
              {"one — usually because a drive or share was off-line when it was "}
              {"stacked. All of those subs are readable again, so it was stacked "}
              {"afresh from the full set. Nothing was deleted; the older pictures "}
            ...
            <Stack gap={0} mt={4}>
              {healed.map((h) => (
                <Text size="xs" key={h.target}>
                  ...
                  {`: was ${h.previous} subs, now ${h.frames} (best before: ${h.best}).`}
                </Text>
              ))}
            </Stack>
          </Alert>
