...
def test_e2e_auto_reject_turns_the_drizzle_pass_on_itself(tmp_path):
    """THE walk-away regression: an unattended drizzled stack sets ``auto_reject``
    and nothing else, and until v0.271.1 that meant **no rejection at all** —
    ``_resolve_auto_reject`` returns untouched under drizzle, and nothing ever set
    ``drizzle_reject``. So every satellite and plane trail went straight into the
    owner's final image. Same trail as ``test_e2e_satellite_trail_rejected``,
    but reached the way the watcher actually reaches it."""
    spec = [
        {"seed": 7, "noise_seed": 100 + i, "n_stars": 10, "streak": (i == 8)}
        for i in range(16)
    ]
    ...
    for auto in (False, True):
        proj = _build_project(tmp_path / f"auto_{auto}", [dict(s) for s in spec])
        try:
            imgs[auto] = _run(proj, auto_reject=auto, output_name="autotrail")
        finally:
            proj.close()
    ...
    # No auto flag → single-pass drizzle, trail intact (today's behaviour for a
    # manual stack that ticked nothing).
    assert deltas[False] > 150.0, f"plain drizzle should keep the trail, got {deltas[False]}"
    # auto_reject on → the two-pass drizzle reject runs and takes it out.
    assert abs(deltas[True]) < 60.0, f"auto should reject the trail, residual {deltas[True]}"


def test_e2e_auto_reject_records_the_drizzle_rejection_it_ran(tmp_path):
    """It self-documents like any other rejection pass, so History can say what
    happened on a run nobody watched."""
    from astropy.io import fits

    spec = [{"seed": 7, "noise_seed": 400 + i, "n_stars": 8} for i in range(6)]
    proj = _build_project(tmp_path / "autoprov", spec)
    try:
        res = run_stack(proj, StackOptions(
            drizzle=True, drizzle_scale=1.0, drizzle_pixfrac=1.0,
            auto_reject=True, background_flatten=False,
            suppress_hot_pixels=False, max_workers=2, output_name="autoprov",
        ))
    finally:
        proj.close()
    with fits.open(res.fits_path) as hdul:
...
def test_auto_reject_never_touches_a_non_drizzled_run():
    """The standard path already resolves ``auto_reject`` into κ-σ / min-max;
    ``drizzle_reject`` must stay off there (it is a drizzle-only pass)."""
    from seestack.stack.stacker import _auto_drizzle_reject

    opts = StackOptions(auto_reject=True, drizzle=False)
    assert _auto_drizzle_reject(opts, 20, (320, 480), None) is False


def test_auto_reject_honours_an_explicit_choice_either_way():
    """A user (or a saved per-target default) who picked ``drizzle_reject`` gets
    exactly that — on stays on below the frame gate, off-with-no-auto stays off."""
    from seestack.stack.stacker import _auto_drizzle_reject

    explicit_on = StackOptions(drizzle=True, drizzle_reject=True, auto_reject=False)
    assert _auto_drizzle_reject(explicit_on, 2, (320, 480), None) is True
    no_auto = StackOptions(drizzle=True, drizzle_reject=False, auto_reject=False)
    assert _auto_drizzle_reject(no_auto, 20, (320, 480), None) is False


def test_auto_reject_declines_rather_than_busting_the_memory_budget():
    """Two-pass rejection holds ~7 canvas planes against the single pass's 4, so
    auto-enabling it on a big drizzled mosaic could turn a run that produces a
    picture today into a hard memory refusal. It must decline instead — the
    picture the owner gets is more important than the pass we'd like to run."""
    from seestack.stack.stacker import _auto_drizzle_reject, _estimate_peak_bytes

    big = (6000, 8000)
    opts = StackOptions(drizzle=True, drizzle_scale=1.5, auto_reject=True)
    single, _ = _estimate_peak_bytes(big, drizzle=True, drizzle_scale=1.5)
    two_pass, _ = _estimate_peak_bytes(big, drizzle=True, drizzle_scale=1.5,
                                       drizzle_reject=True)
    # A budget the single pass fits and the two-pass one doesn't.
    budget_gb = (single + two_pass) / 2 / 1e9
    assert _auto_drizzle_reject(opts, 40, big, budget_gb) is False
    # Room for both → it takes the rejection.
    assert _auto_drizzle_reject(opts, 40, big, two_pass / 1e9 * 1.1) is True
