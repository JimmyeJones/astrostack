...
It found one real defect, shipped as **v0.434.2**.
...
it is a nebula) and **v0.429.3** (`nightplan._depth_only_picks`: "Worth more
...
*(Both were **committed** as v0.428.3 / v0.428.4 and renumbered at merge time,
per §11's "choose the number at merge time, from `main`": the
`…-8i7zw0` run merged v0.429.0–v0.429.1 while this branch was sitting in CI.
Grep the commit subjects by their code identifiers —
`nextBestMove.integrationBars`, `nightplan._depth_only_picks` — not by version.
No collision on the work itself: the two runs touched different files, which is
what small single-topic branches are for.)*
