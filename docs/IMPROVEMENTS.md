...
- **⚪ MEASURED RESIDUAL OF v0.409.0 (Builder 2026-09-10, filed with the fix that closed the gradient half) —
  a mosaic's per-panel *steps* still inflate the sky level Auto sets its stretch target from.** *(Severity: low —
  measured at `target_bg` 0.230 single-field vs 0.210 for the identical stack laid out as a mosaic, i.e. ~9 %,
  against the 22 % the gradient half was worth. Confidence: HIGH, same harness. Size: M.)*
  On `tests/test_auto_noise_measure.py::_scene(0.004, seed=13)` the reported sky is **0.024** as a single field and
  **0.075** as a mosaic (0.082 before v0.409.0). A degree-2 `fit_sky_poly` surface is the right shape for light
  pollution and the wrong shape for a *step*, so the detrend can only partly absorb panel offsets — by design, not
  by oversight. **The honest fix is not a better surface**: `background.level_coverage` already removes those steps
  and already runs ahead of the stretch on exactly these stacks, so the level should be measured per coverage
  level, which means `analyze_proxy` (and `auto_recipe`, which calls it) needs the coverage map it is currently not
  handed. That is a signature change through `build_auto_recipe_for_run` / `build_auto_analysis_for_run`, which is
  why it was not folded into v0.409.0. **Before starting, read the v0.409.0 entry in [`SHIPPED.md`](SHIPPED.md)**:
  the σ half is deliberately left measured on the raw pixels and must stay there, and every constant in
  `auto_recipe` is calibrated — this is a measurement fix or it is nothing.
