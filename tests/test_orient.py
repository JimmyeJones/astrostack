...
@pytest.mark.parametrize("angle", [0.0, 90.0, 180.0, 270.0, -90.0, 89.6, 30.0,
                                   123.0, -37.5, 45.0])
@pytest.mark.parametrize("size", [(60, 40), (40, 60), (50, 50)])
def test_canvas_width_in_preview_px_survives_the_rotation(angle, size):
    """The scale bar is held as a fraction of the *canvas* width, so a preview a
    save already rotated has to be measured by how much of itself the canvas
    width now spans. Round-trip: rotate a canvas-sized picture, then ask — the
    answer must come back to the canvas width, including at 45° where inverting
    the expand box directly would be hopeless.

    The expand canvas is sized with ``ceil``/``floor``, so it carries up to a
    couple of pixels of rounding the answer inherits — hence the ±2 px on these
    deliberately tiny canvases; the test below pins the *relative* error on a
    preview-sized one."""
    from seestack.render.orient import (
        canvas_width_in_preview_px,
        north_up_pixel_transform,
    )

    w, h = size
    _, _, new_w, new_h = north_up_pixel_transform(w, h, angle)
    got = canvas_width_in_preview_px(w, h, new_w, new_h, angle)
    assert got is not None
    assert abs(got - w) <= 2.0


@pytest.mark.parametrize("angle", [30.0, 45.0, 123.0, -37.5])
def test_canvas_width_in_preview_px_is_sub_pixel_on_a_real_preview(angle):
    """On a real 1024 px preview the bounding box's rounding is noise: the span
    lands within a tenth of a percent, so the drawn bar keeps its true length."""
    from seestack.render.orient import (
        canvas_width_in_preview_px,
        north_up_pixel_transform,
    )
    ...
    _, _, new_w, new_h = north_up_pixel_transform(w, h, angle)
    got = canvas_width_in_preview_px(w, h, new_w, new_h, angle)
    assert abs(got - w) / w < 0.001


def test_canvas_width_in_preview_px_is_exact_for_the_snapped_case():
    """A near-orthogonal angle is a pure axis swap, so no rounding is allowed —
    on a quarter-turn the canvas width is the picture's *height*."""
    from seestack.render.orient import canvas_width_in_preview_px

    assert canvas_width_in_preview_px(60, 40, 60, 40, 0.4) == 60.0
    assert canvas_width_in_preview_px(60, 40, 40, 60, 89.6) == 60.0
    assert canvas_width_in_preview_px(60, 40, 60, 40, 180.0) == 60.0
    assert canvas_width_in_preview_px(60, 40, 40, 60, -90.0) == 60.0


def test_canvas_width_in_preview_px_follows_the_preview_downscale():
    """The preview is a downscale of the canvas, so the answer is in *preview*
    pixels: a half-size 90°-rotated preview spans half the canvas width."""
    from seestack.render.orient import canvas_width_in_preview_px

    assert canvas_width_in_preview_px(120, 80, 40, 60, 90.0) == 60.0
    assert canvas_width_in_preview_px(120, 80, 60, 40, 0.0) == 60.0


def test_canvas_width_in_preview_px_declines_a_degenerate_size():
    """No canvas or no picture has no answer — the caller keeps the width it has
    rather than working from a made-up number."""
    from seestack.render.orient import canvas_width_in_preview_px

    assert canvas_width_in_preview_px(0, 40, 60, 40, 30.0) is None
    assert canvas_width_in_preview_px(60, 40, 60, 0, 30.0) is None
