...
- **NEW IDEA (Builder 2026-08-18, the pattern behind both v0.266.1 and the hints added in v0.267.0) — a Tooltip is
  ...
  **▶ SIXTH SLICE SHIPPED — v0.413.1, the planning surfaces, which are the most phone-critical in the app**
  ...
  UTC).
  **Still open after both:** the ~30 remaining non-control anchors — `NightsCard` ×3, `BestMonthsStrip`,
  `MosaicMapCard`, the editor's `OpList` chips, `Editor`'s `SlowPreviewChip`, `Target`'s integration/streak/
  trailed badges, `BestPictures`/`Gallery`/`Calibration`/`LifeList`, and `ImageLightbox`'s bare-icon anchors.
