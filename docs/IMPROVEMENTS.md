...
- **v0.448.0** — 🟠 BUG (trust — PRIORITY 1-adjacent (the displayed picture) / 3, observer issue [#903](https://github.com/JimmyeJones/astrostack/issues/903)): **"Reprocess everything" with the auto-edit switch off silently replaced every finished picture on the wall with a flat linear stack.** New `pipeline._picture_is_auto_finished` asks, per target and before the restack, whether the picture the target displays is an auto-edit the app baked (`editor_auto_baked_look` stamp on the newest previewed run, no pinned cover) and carries that finish onto the fresh run — reported as `kept_finished` and worded by `reprocessSummary`; plus the dialog copy (`Settings.tsx` `editNote`) names what leaving the switch off does to the Library wall, life list and sky map. Options (2) and the cover-pin form of (3) deliberately declined, with reasons, in `SHIPPED.md`. One named residual: a hand-saved edit is not carried forward (Auto's look is not evidence of what its author wanted) — the open "finished / not stretched yet" wall-chip idea is the standing general answer.
