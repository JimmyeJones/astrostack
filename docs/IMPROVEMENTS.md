...
- **LEAD (Builder 2026-09-19, filed while shipping v0.470.0 — the two things that fix left unmeasured, and
  neither should be blind-built)** *(Pillar: image quality + trust — PRIORITY 1/4; size S each to write, **M
  to be sure of**; both real-data-gated.)*
  **(a) `grain_region` is coarse on purpose, and one shape falls between its two words.** The word says which
  *rule* found the thin region — a coverage plateau (`"panel"`, a frame's footprint by construction) or a
  depth ramp (`"spread"`). A canvas that ramps **and** carries a genuinely under-shot panel reads as
  `"spread"` and gets the softer sentence, which withholds *"another night on that panel"* from someone who
  would benefit from it. A geometric classifier was built and dropped this run — distance inside the covered
  footprint, normalised by the largest inscribed radius, which is the measure the observer's own control #2
  used — because on this repo's fixtures it **cannot tell a corner panel from the corner it sits in**: it read
  `_uneven_canvas`, a stripe against the frame edge and a panel by construction, as an edge. Do not re-file
  that shape without a discriminator that survives it. The honest next step is the owner's own library: take a
  real `<T>_mosaic_sub/` run, compute the thin band, and ask whether its connected components separate into
  *plateaus at a panel's depth* and *ramps* at all — if they do, the depth histogram *within* the band is the
  discriminator, not its geometry.
  **(c) the heal reads strided, and `_GRAIN_MIN_SKY_PIXELS` is an absolute floor.** `backfill_coverage_grain`
  is how the owner's already-stacked mosaics pick the new measurement up, and it strides their masters
  (`_seam_read_step`: 3 on a 3494x2470 canvas, capped at 4). The thin band's *sky* sample shrinks with the
  square of the stride, and the 500-pixel floor does not, so a large enough canvas can decline where a smaller
  one answers — reproduced on this repo's own fixture, which measures at strides 1 and 2 and returns `None` at
  4. **The evidence says it does not bite on the real thing:** the observer measured all 26 mosaics "at the
  stride `backfill_coverage_grain` itself would use" and got a ratio for every one. So this is filed as a
  *watch*, not work: if a future report shows a mosaic still NULL after a heal, the floor is where to look
  first, and the honest fix is to scale it by `step²` (it is a sample-size argument, and 500 full-res pixels
  is 31 strided ones) rather than to lower it outright. Do not touch it without a case.
  **(b) the deep band's reference is unmeasured on a canvas whose median sits in the overlap strips.** The
  band rule takes "deep" as within ±15 % of the picture's median depth. On a mosaic with real overlap the
  median can sit in the 2–4× overlap band rather than on a panel body, in which case a single-covered panel is
  "thin" — which is arguably right (it *is* grainier) and arguably names most of the picture. The observer's
  26 measurements are the only evidence and they are this rule's own, so they cannot check it. What would:
  compare `coverage_median_depth` against `coverage_trim.panel_coverage_level` on the owner's real mosaics —
  if the two diverge by more than ~1.5× on a substantial share of them, the reference is worth a second look.
  Until then the figures are conservative (the clipped σ can only understate) and the sentence prescribes
  nothing that could be harmful.
...
- **v0.470.2** — 🟠 TEST + WATCH (Builder, the same run, from asking how v0.470.0 actually *reaches* the owner's already-stacked mosaics): **the heal reads strided, so the band answer had to be pinned across a stride, and one floor is worth watching.** `backfill_coverage_grain` is the only path by which the owner's 26 mosaics pick the new measurement up, and it reads their masters at `_seam_read_step` (3 on a 3494×2470 canvas, capped at 4) — so a healed row and a freshly stacked one must be the same kind of number or the library ends up holding two currencies. Measured on the ramping fixture at stride 2: the depths and the share are **exact** (read off integer counts) and the ratio drifts 6 %, the same few percent the per-level branch's own stride test allows; a test pins all four. The fixture *does* decline at stride 4, on `_GRAIN_MIN_SKY_PIXELS` — an absolute 500-pixel floor against a sky sample that shrinks with the square of the stride — which is filed as a **watch** on the lead rather than as work, because the observer measured all 26 real mosaics at the stride the heal itself would use and got a ratio for every one. If a future report shows a mosaic still NULL after a heal, that floor is where to look first, and scaling it by `step²` is the honest fix. Tests +1; no production code touched.
- **v0.470.1** — 🟠 BUG (trust — PRIORITY 3, Builder-found the same run by grepping for the *other* readers of the value v0.470.0 had just given a new word to): **the third surface that prescribes from the grain measurement was still offering the night.** `integrationTrend`'s plateau branch reads the same `grain_verdict` off the same run as the health note and ends *"…so another pass over it is the thing left worth shooting here"* — which on a ramping canvas is the identical false prescription v0.470.0 exists to remove, on the one card whose whole subject is what is still worth shooting. It now takes the same fork as its two siblings off the same `grain_region`: on a `"spread"` it offers the crop and keeps every other clause; a `"panel"` and an older backend that sends no word keep the sentence byte for byte. No plumbing needed — `IntegrationTrendBadge` passes the run rows straight through and they already carry the field. Frontend only, +2 vitest, **fail-before verified by scratch revert**, and a test pins that the hours and the exponent are identical either way. Full entry in [`SHIPPED.md`](SHIPPED.md).
- **v0.470.0** — 🟠 BUG (image quality + trust — PRIORITY 1/4, from observer issue [#952](https://github.com/JimmyeJones/astrostack/issues/952), reproduced on a ramping fixture before it was fixed): **the mosaic grain measurement had no plateau to stand on, so it said nothing about any real mosaic — and one chip filled the silence with "Panels even".** `measure_coverage_grain` chose its thin region by asking which **single integer coverage level** held a tenth of the canvas; a mosaic dithered and re-pointed across many nights has no such level, and the observer measured **79–1,392 distinct levels per canvas with zero clearing the bar on 26 of 26** of the owner's mosaics. So `grain_ratio` was non-NULL on 2 of 680 runs, both single fields, and the grain note, `PanelSeamsBadge`, `nextBestMove`'s good rung, `integrationTrend`'s plateau branch and `grainProjection` were all inert — while two runs showed a green *"Panels even — you shouldn't see seams between them"* over canvases measuring **1.86× grainier across 43.8 %** and 1.73× across 39.4 %. Fixed by asking the question twice: the per-level rule is unchanged and still answers first (so every canvas with a plateau, including every fixture in this repo, is byte-for-byte what it was), and when it finds nothing a **band** rule takes thin and deep as fractions of the picture's own median depth — deliberately the banding the observer measured with, which reads 1.39–1.86× over a median 28.8 % on those same 26. Plus a guard on the one run that *did* fire: it compared **3,116 subs against 3,117** and reported 1.008, so a candidate level must now be at least a tenth shallower than the modal one. **Switching the measurement on would have shipped a wrong sentence**, so the answer carries a new `grain_region` (`"panel"` = a plateau, which is a frame's footprint by construction; `"spread"` = the depth ramps, so part of the thin part is the outline the pointings leave and no amount of further shooting narrows it): a plateau keeps *"another night on that panel"*, a ramp offers the crop instead, on the health note and on both prescribing surfaces of the Target page. And the ragged-border offer stops promising *"a clean, even rectangle"* on a measured spread — `largest_covered_rect` keeps 37 % of one of his mosaics and 19.5 % of what it keeps is still 1.58× grainier — with the grain note standing down so one thin part is not two percentages in two notes. Tests +12 Python / +4 vitest; **fail-before verified by six scratch reverts**. Additive nullable column with no `SCHEMA_VERSION` bump (the `seam_scale` shape), additive FITS card and API field; no on-disk, default or threshold change on any canvas that already had an answer. Full entry in [`SHIPPED.md`](SHIPPED.md).
