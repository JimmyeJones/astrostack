...
def test_auto_edit_clears_a_north_up_rotation_it_just_rendered_away(
    client, solved_library
):
    """An auto-edit rewrites the run's preview from the FITS grid, so a North-up
    rotation an earlier "Adjust → North up → Save" baked into those bytes is gone
    — and the angle recorded beside them must go with it.

    Left behind it would be a *ghost*: the Sky map turning the coverage mask and
    the tile WCS by an angle the picture no longer carries, History withholding
    its object pins for a rotation that isn't there, the share download's "North
    up" quietly doing nothing.

    Not reachable today — every production call site auto-edits a **fresh** run,
    whose column is still NULL — so this pins the invariant rather than fixing a
    live bug: whoever rewrites a run's preview bytes owns the angle recorded
    beside them. Three separate consumers have already been broken by "nothing
    records what these bytes are"; this is the writer's half of that."""
    from seestack.io.library import Library
    ...
    body = _wait_job(client, r.json()["job_id"], timeout=120)
    assert body["state"] == "done", body
    rid = body["result"]["stack"]["run_id"]

    lib = Library.open_or_create(solved_library / "library")
    try:
        # Stand in for the user saving this run's picture North-up afterwards.
        proj = lib.open_target("M_42")
        try:
            assert proj.set_stack_preview_north_up(rid, 90.0)
        finally:
            proj.close()
        ...
        proj = lib.open_target("M_42")
        try:
            run = next(x for x in proj.iter_stack_runs() if x.id == rid)
            assert run.preview_north_up_deg == 0.0
        finally:
            proj.close()
    finally:
        lib.close()
