...
_Builder audit log 2026-07-11 (baseline green: 1113 passed / 2 skipped): with both open Bugs entries REAL-data-gated
and the Ideas backlog shipped-or-gated, dogfooded the out-of-box Auto path end-to-end (healthy — sky lifted to ~0.17,
fully finite, per-channel sky medians match near-exactly preview↔export) and ran **three parallel adversarial audits**
required to *reproduce* any suspected defect: (1) the **stacker combine path** (`stack/{stacker,accumulator,weighting,
photometric,output}.py`), (2) the **editor render pipeline** for proxy↔export parity (`edit/pipeline.py`, `recipe.py`,
`registry.py`, `ops/*`), and (3) the **webapp orchestration / autonomy paths** (`webapp/{pipeline,watcher,jobs,
calibration}.py`). **One genuine data-integrity bug found, reproduced, fixed, and shipped this run — v0.109.17**
(calibration master id/filename reuse after deleting the newest master → a stack run's persisted `dark_path` silently
rebinds to a different master → miscalibrated **Reprocess everything** — see Shipped, and the fixed entry below).
**The other two audits traced clean, with runnable numeric reproductions:** stacker — `add`/`add_window` sibling
accumulators byte-identical under non-unit weights + NaN gaps, NaN=coverage held (no gap→0 leak), every
weighting/photometric/output divisor guarded, photometric scale applied identically in both κ-σ passes, pass-1 Welford
freed before pass-2, memory guard bounds in-flight frames; editor — every full-res pixel measure (`sharpen` radius,
`denoise` sigma_spatial, `deconvolve` psf/ring, `background` box/dilate, `stars.reduce`/`boost_nebula` footprint) is
scaled via `ctx.scaled_px`, ops execute in identical recipe order preview↔export, and NaN gaps survive all 14 ops
(the only unscaled pixel measure remains `background.level_coverage`'s `dilate_object_mask_px=4`, already-filed +
measured-negligible). **Two non-harmful observations noted, not shipped (per §2, no manufacturing):** (a) `webapp/
pipeline.py::submit_process_target` always returns a truthy summary, so a Process job **cancelled during the stack**
is classified `done` (`stacked:True`/`run_id:None`) rather than `cancelled` — `jobs.py`'s `engine_cancelled` check
only inspects a top-level `cancelled` key that `process_target` (unlike `submit_stack`/`reprocess_all`) never sets;
**no data impact** (no run written, no marker set), filed to Ideas → Infra as a low-priority state-classification
tidy-up; (b) the `background.level_coverage` dilation/heavy-stride parity nuances remain as already filed
...
- ~~**Calibration master ids/filenames were reused after deleting the newest master — a stack run's persisted
  ...
  `webapp/calibration.py::_next_id` allocated `max(current ids)+1`, not monotonic across deletions: deleting the
  most-recently-built (highest-id) master freed its id, and the next build reused both the id and the `{kind}_{id}.fits`
  filename. Since stack runs persist the server-resolved calibration *path* in `options_json` and `reprocess_all`
  reuses those options verbatim, a run recorded to `.../dark_1.fits` (over-)subtracted a completely different master
  on reprocess. Fixed with a persisted high-water mark (`calibration/masters_next_id`, kept in its own file so
  `masters.json` stays a bare list → upgrade- and downgrade-safe): `mid = max(registry_max, high_water) + 1`. Reproduced
  end-to-end (delete newest → rebuild → stale path resolves to the wrong master's pixels); regression tests
  `test_master_ids_are_never_reused_after_deletion` + `test_master_id_high_water_falls_back_to_registry_max_when_absent`.
  *(Traced + reproduced, Builder webapp-orchestration audit 2026-07-11; medium severity, data-integrity on the
  unattended reprocess path.)*
...
- ~~**`render.autostretch` crashes on a 2-D (mono) input while its sibling `asinh_stretch` handles it —
  ...
  copy=True)` with **no** 2-D→3-D expansion, then immediately `np.isfinite(img).any(axis=2)` — so a 2-D
  grayscale array raised `AxisError: axis 2 is out of bounds`, while its sibling `asinh_stretch` already
  guards exactly this (`if img.ndim == 2: img = np.stack([img,img,img], -1)`) and the module docstrings assert
  the two behave "exactly" alike. Both are public, re-exported API. Took the one-line guard (identical to the
  sibling's) so `autostretch` now treats a mono array as grey and returns a neutral 3-channel result. Every
  in-repo caller feeds 3-D today (mono stacking expands to 3 channels at `stacker.py:1471`, the compare dialog
  up-converts, `generate_thumbnail` feeds 3-D), so this is a pure robustness/consistency fix with no
  behaviour change for existing callers — but it closes the latent API-contract gap for any future
  mono/FITS-preview path. Regression test `tests/test_nan_aware_stretch.py::test_autostretch_accepts_2d_mono_like_asinh`
  (a 2-D input crashes with `AxisError` before / stretches to an identical-channel grey result after, and
...
`calibrate/apply.py`, `render/thumbnail.py` (asinh + STF stretch, NaN-preserving stride/downsample),
`bg/coverage_leveling.py`, `edit/{proxy,pipeline}.py`, and the v0.99.4–0.99.7 watcher recovery logic
(`webapp/watcher.py` + `main.py::_on_batch_ready`/`_stranded_batch_needs_retry`). Read for
NaN=coverage violations, photometric sign, calibration double-subtract, weight/scale cross-pass symmetry,
proxy↔export parity, and the stranded-batch retry-flag state machine. **No new verified bug found** —
photometric scale multiplies by `ref/transp` (dim frames gain-matched *up*, correct); `apply_raw` subtracts
dark **or** bias (never both) and never mutates its input; the min/max-k accumulator's ±inf-seeded k-sets and
graceful count<2k+1 degradation are sound; `asinh_stretch`/`autostretch` compute all stats over finite
pixels and render NaN black; `build_proxy` and `load_coverage` stride by the same integer step so the
coverage-leveling op lines up proxy↔full-res; and the watcher's single-retry recovery flag can't loop
(`watcher_pipeline_is_recovery` is consumed then reset in `_on_batch_ready`, re-armed only on a genuine
`error`, never a `cancel`). **Numeric dogfood** of the real single-field OSC path (12 dithered subs → κ-σ
quality-weighted `run_stack` → one-click `auto_recipe` → preview vs `_render_recipe_fullres` export):
`is_mosaic` persists **False**, the Auto chain is the full sane sequence
(final_gradient → color_calibrate → stretch → scnr → saturation → curves → sharpen; denoise correctly
absent on a clean stack), **preview↔export parity exactly 0.000%** at proxy_scale 1.0, RGB medians balanced
(0.258/0.220/0.259), median grey 0.242, and true (all-channel) black-clip only **0.046%** — healthy.
**One traced trust/friendliness gap filed to Ideas (NOT a bug — no wrong *result*, and only reachable by
