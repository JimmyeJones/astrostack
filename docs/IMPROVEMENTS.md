...
- **v0.478.3** — 🔴 §10 (raw-data safety) + public-repo privacy, from the 2026-09-26 setup audit: **the layout guard
  ...
  (reproduced). `config.nested_incoming_conflict` now refuses it on save via `LIBRARY_MANAGED_SUBDIRS` (pinned to
  `_TARGETS_SUBDIR` / `CALIBRATION_SUBDIR`); an install already in that layout still boots and warns (§9). Two new
  tests fail against the old guard. Same PR: **22 agent commits had reached `main` with the owner's personal email
  as author** — `scripts/agent-setup.sh` now pins the no-reply identity and installs a pre-push hook
  (`scripts/check-commit-identity.sh`, an allowlist) that refuses anything else, CI gained a `Commit identity`
  job, and AGENTS.md §10 the rule; and CI no longer cancels `main` runs (`cancel-in-progress` on PRs only). Full
  suite 6,633 passed.
