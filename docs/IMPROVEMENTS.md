...
- **LEAD (Builder 2026-09-25, filed while shipping v0.473.1 — the two halves of observer issue
  ...
  **(c) `n_roughly_aligned` is NULL on all 690 of the owner's stack runs** — the column that exists to record
  frames that stacked only roughly aligned, i.e. the one surface that would have shown any of this. Worth a
  grep before anything else here: it may simply never be written.
