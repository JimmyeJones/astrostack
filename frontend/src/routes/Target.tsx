...
// Pre-flight "this batch looks like two targets" guard. A Seestar's field of
// view is ~1.3° across; dithering nudges a pointing by arc-minutes and a mosaic
// steps adjacent panels ~1° apart (they overlap), so *one* target's solved
// frames — a single pointing, a dithered set, or a contiguous mosaic — form a
// chain whose neighbours are all within a couple of degrees. Two *different*
// targets accidentally dropped in one incoming folder sit many degrees apart
// with nothing bridging the gap. If the user stacks such a batch, the stacker
// picks one pointing as the reference and silently drops every frame whose
// footprint doesn't overlap it (the NALIGNFL count) — so half the night is
// wasted on a stack the user only discovers is half-complete afterwards.
//
...
export function detectMixedPointings(frames: Frame[]): MixedPointings | null {
  ...
  const vecs = pts.map((f) => raDecToVec(f.ra_center_deg as number, f.dec_center_deg as number));
