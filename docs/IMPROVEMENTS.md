...
- **v0.407.2** — 🐛 BUG (trust / friendliness, PRIORITY 3; Builder-verified by computation against the two helpers as they stand): **the Target page's "is more time worth it?" line quoted the ideal √t curve at a target its own stacks had already measured off it.** `grainProjection` projected only along σ ∝ 1/√t — right from *one* measured point, wrong the moment there are two — while `integrationTrend`, on the same target's runs, had already fitted the real falloff exponent and was printing it on History's "Noise trend" card. Measured on three ordinary two-stack histories: 1 h σ 0.060 → 3 h σ 0.050 fits **p = 0.166**, where the card said *"6.3× the light — roughly 16 h more"* and the target's own rate says **250×, about 747 h**; the other two under-quoted 21 h against 88 h and 48 h against 112 h. Sixteen hours is four or five clear nights a beginner would actually go and shoot. The existing stand-down did not catch it: `cardGrainProjection` goes quiet only on a **plateaued** verdict (p ≤ 0.15), so every "slowing" target kept the ideal answer. `grainProjection` now projects along `min(IDEAL_EXPONENT, p)` — clamped **one-sidedly**, because a p above 0.5 is jitter and never licence to promise less light than shot noise needs, while a p at or below 0 is left to fall through to the `beyondReach` copy that was already written for it rather than floored up into a quotable number. The doubling percentage is read off `trend.percentCutIfDoubled` rather than recomputed, so the two surfaces cannot round to two different figures for one target. Copy gains one clause only where the rate is measured (*"At the rate this target's own stacks have been improving…"*). **With one stack — the case this module was built for — every figure and sentence is character-for-character unchanged**, and all fifteen pre-existing cases pass untouched. Frontend-only; no config, schema, on-disk, API-shape or default change. Tests +7, all seven fail before. Full entry in [`SHIPPED.md`](SHIPPED.md).
