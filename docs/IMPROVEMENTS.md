...
- **🟡 SWEPT ONCE, ONE UNTRUTH FIXED (Builder, v0.309.1, branch `claude/compassionate-galileo-x2nj2o`); the
  ...
  **▶ The listings had their first pass 2026-09-13 and it turned up one straight away, on a sixth axis: *a
  ...
  the listings are where a request that the engine declined survives as a claim. `Gallery.highlightBadges`'
  three remaining chips are the obvious next reads: `background_flatten` (the stack path degrades to **no**
  subtraction when any channel's fit fails, v0.173.2), `final_gradient_removal` (auto-enabled on a mosaic,
  so the chip *under*-claims there — harmless, but it means the chip is not a record of what ran), and
  `lucky_fraction`. None was checked this run.
