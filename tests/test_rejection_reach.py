...
def test_drizzle_reports_its_own_two_pass_gate_and_says_nothing_when_unasked():
    ...
    on = rejection_reach(StackOptions(drizzle=True, drizzle_reject=True), 200)
    assert on.reaches is True and on.lone_outlier_min_frames == 4
    too_few = rejection_reach(StackOptions(drizzle=True, drizzle_reject=True), 3)
    assert too_few.reaches is False
