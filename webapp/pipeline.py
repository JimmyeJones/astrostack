...
def _picture_is_auto_finished(lib: Library, safe: str) -> bool:
    ...
    * the newest run carrying a preview was auto-edited by **us**, i.e. it has an
      ``editor_auto_baked_look`` stamp. A saved recipe with no stamp is somebody's
      own work, and Auto's look is no evidence of what they wanted — there we
      stand down, and the reprocess dialog names the consequence instead.
    ...
    try:
        ...
        try:
            for run in proj.iter_stack_runs():  # newest first
                # The wall shows a *preview*, so a run without one is not the
                # picture in question — skip past it rather than answering for it.
                if not run.preview_path:
                    continue
                return bool(proj.get_meta(
                    f"{AUTO_EDIT_BAKED_LOOK_PREFIX}{run.id}"))
        finally:
            proj.close()
