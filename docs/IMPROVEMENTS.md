...
- **LEAD (Builder 2026-09-17, filed with the v0.455.2 fix that found it without the tooling's help) — no
  dogfood pass has ever held the owner's *scale*, and the probe measures the one dimension that hides it.**
  *(Pillar: maintainability in service of not missing bugs — size M; **read the cost note before starting**.
  Confidence: the hole is measured, not reasoned — see v0.455.2 in [`SHIPPED.md`](SHIPPED.md).)*
  Every pass ever recorded runs a sample of **six** subs and two or three targets. The owner's library is
  **104 targets, 26 of them mosaics, 64 nights, 271 GB, largest target 35,894 subs** (5,477 on the one
  `webapp/estimate_cache.py` was measured at). So every surface whose cost or shape is a function of *how much
  he has* has only ever been exercised at a size where nothing can go wrong — and the probe cannot see it
  either, because it measures **page height**, and the frames table that carried this defect lives inside a
  `mah="65vh"` scroll container whose height is by construction independent of its rows. That is the
  missing-observing-site (v0.436.1) / empty-`incoming/` (v0.442.0) / click-only-Compare (v0.440.2) /
  no-master-dark (v0.455.1) hole one layer down: not a *state* the tooling never reached, a *magnitude*.
  **What it found on the way in, without the flag existing:** `Target.tsx` rendered one table row per sub,
  ~22 DOM nodes each — **120,727 nodes and 41 s to first paint** on 5,477 subs, in a table that shows twenty
  rows, on a page opened from a phone. Fixed as v0.455.2. That was reached by code-reading plus a jsdom
  measurement, which is exactly the kind of evidence a flag should be producing instead.
  **The shape worth building, and the one real cost.** A `--deep` flag seeding one target with thousands of
  subs, plus a probe line that prints **DOM node counts** for the page's heaviest containers beside the
  heights it already prints — a count is the measure that moves with the rows, and a height is the measure
  that does not. **Cost is the whole design question:** the sample's lights are 480×320 and ~300 KB each, so
  5,477 of them is ~1.6 GB of scratch and a stack nobody will wait for. So do **not** reuse the light writer:
  either write much smaller frames (the surfaces under test read frame *rows*, not pixels), or seed the rows
  and skip the stack — and say in the flag's own output which it did, because a pass that quietly stacked six
  subs while claiming five thousand is the empty state wearing a better hat (the mistake v0.455.1's own first
  run made). **Also check before building:** `AutoGradeModal`'s recommendation table renders one row per
  flagged frame under a 25 % cap (~1,369 rows on this owner's biggest target, ~6 nodes each, modal-gated) —
  that is the only other unbounded per-sub render left in the frontend, and it is small enough that it is
  probably a note rather than a fix. `Stack.tsx` reads the same complete list and renders none of it.
