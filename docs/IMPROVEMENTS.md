...
- **LEAD (Builder 2026-09-12, seen in the first browser measurement of `/tonight` that had rows in it;
  NOT fixed, because this run had already taken its one IA slice) — the "Add more to what you're shooting"
  table loses its last column to a target's own `safe_name`.** *(Pillar: friendliness — PRIORITY 3; size S.
  Confidence: photographed at 420 px on the bundled samples, `$SHOTS/phone_tonight.png` on a `--mosaic`
  pass.)*
  Both tables on the page are the same `TargetTable`, but the library one renders
  `Sample_Orion_Nebula_M42 — Sample: Orion Nebula (M42)` where the catalog one renders `M32`, so its Target
  column takes three lines and pushes "Time up" out of the `Table.ScrollContainer` — the header reads "T"
  over "u" and the values one character per line. **This is not the v0.436.2 badge bug**: nothing is
  ellipsised inside a fixed box, the table really is wider than the phone and a swipe does reveal it, which
  is why the overflow and CLIPPED-LABEL probes both (correctly) stay quiet. It is still ugly, and it is the
  *library* half — the rows the owner is most likely to be reading. **The shape, if it is worth it:** the
  duplicated `id — name` is the whole cause (`TargetRow`'s `label` prints both when they differ), and for a
  library row the two are the same object said twice; showing the friendly name with the safe-name as the
  link target would halve the column. **Care:** the catalog rows genuinely need both (`M31 — Andromeda
  Galaxy`), so this is a change to the *already-targeted* branch only, and `splitTargets` is where the two
  are already told apart.

- ~~**LEAD (Builder 2026-09-12) — the bigger-print nudge recommends Drizzle without knowing whether *this*
  picture has the subs for it.**~~ — **✅ SHIPPED v0.436.0** (Builder 2026-09-12); entry cut to
  [`SHIPPED.md`](SHIPPED.md), one-liner under "Shipped" below. It was worth it, and it turned out to be a
  *bug* rather than the copy tweak the lead sized: the Stack form's own print panel claimed in a comment that
  it could "never recommend the thing it would then warn against" while reading the target's **total** where
  the caution beside it reads the **per-pixel** depth — identical on a single field, wrong on every mosaic.
  The lead's "care" note about a baked print file was checked and does not apply: the baked file carries
  `print_advice`, which never names Drizzle.
