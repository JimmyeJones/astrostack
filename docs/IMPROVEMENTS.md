...
- **LEAD (Builder 2026-09-13, filed with v0.438.10 because it is the *other* thing a re-render's own
  geometry says about light it did not collect) — a cropped export reports its depth against the canvas
  it has left, not the one the subs were spread over.** *(Pillar: trust — PRIORITY 3; size S to measure,
  S–M to fix; **measure before building — half of this is arguably already right**.)* The wall (and the
  Gallery card, and the History trend) scale a run's totals by `field_fulls_of_sky(canvas_w, canvas_h, …)`
  — canvas area ÷ one native frame — to read integration and frame count **per pixel**. An editor export
  records the canvas it actually wrote (`canvas_h=out.shape[0]`, `_apply_editor_to_run`), which a crop
  makes *smaller*, so its `field_fulls` falls and its apparent per-pixel depth rises by exactly the crop
  factor. Reachable on every edited mosaic, because Auto seeds a border trim on first open (v0.390.0) and
  keeps ~92 % of the bundled 2×2 — an ~8.6 % inflation of two of the four ranked metrics.
  **Why it is filed rather than fixed, and the distinction any fix has to make:** trimming a **ragged
  border** genuinely leaves a deeper picture behind, so there the higher number is closer to the truth, not
  further from it — the honest reading of the trim case may be today's. A **content** crop is the wrong
  half: cropping to the galaxy removes no subs from the pixels that survive, so the depth is unchanged and
  the inflation is pure. On a **single field** it cannot fire at all (`per_pixel_total` clamps
  `field_fulls ≤ 1.0` to "no scaling"), so this is a mosaic-only question — the frontier §1 names.
  **Measure first:** take the bundled mosaic sample, stack it, and compare `coverage_median_depth` on the
  *cropped* region against `n_frames_used / field_fulls` computed from the cropped canvas, for an
  Auto-trim crop and for a crop-to-one-panel. If the trim case agrees and only the content crop diverges,
  the fix is to scale a re-render by the **source run's** `field_fulls` and not its own — the same
  "a re-render's own row is not a measurement of its light" rule `stacking_coverage_max` states — and it
  belongs beside it in `webapp/derived_light.py`. If both diverge, this is one rule, not two. Do **not**
  flip it blind: it moves the ranking of every edited mosaic on the wall.
